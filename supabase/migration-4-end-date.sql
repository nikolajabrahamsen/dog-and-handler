...
alter table classes add column if not exists ends_at timestamptz;

-- Replaces the previous version: also blocks registration once a class's
-- end date has passed, even if someone still has an old link to it.
